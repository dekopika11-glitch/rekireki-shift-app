...
export default function Home() {
  ...
  useEffect(() => {
    const fetchExistingShifts = async () => {
      // ★修正: 誰かを選んだ時点で必ず備考欄をリセットする（前の人の書きかけなどを消す）
      setMonthlyRemark(""); 

      if (!staffName) { setShifts({}); return; }
      ...
      // ★修正: ここにあった「過去の備考を読み込む処理」を完全に削除しました
    };
    fetchExistingShifts();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ★修正: 備考欄に文字が入力されている場合のみ上書き保存する
      if (monthlyRemark.trim() !== "") {
        await (supabase.from('monthly_remarks') as any).delete().eq('staff_id', currentStaffId).eq('year_month', ym);
        await (supabase.from('monthly_remarks') as any).insert([
          { staff_id: currentStaffId, year_month: ym, remark: monthlyRemark.trim() }
        ]);
      }
      
      alert("提出完了！");
      // ★修正: 送信が完了したら、入力欄の文字を空っぽにリセットする
      setMonthlyRemark(""); 
    ...
    days.push(
      <div key={i} className={`border-r border-b flex flex-col items-center h-36 overflow-hidden ${isHoliday ? 'bg-red-50' : 'bg-white'}`}>
        <div className="flex justify-center w-full mt-1 mb-0.5">
          <span className={`font-bold text-sm flex items-center justify-center ${isToday ? 'bg-black text-white w-6 h-6 rounded-full' : (isHoliday ? 'text-red-600' : 'text-gray-800')}`}>{i}</span>
        </div>
        {isHoliday ? (
          <div className="flex-1 flex items-center justify-center w-full px-1"><span className="text-red-400 font-bold text-[10px] sm:text-xs text-center truncate">定休日</span></div>
        ) : (
          <div className="w-full flex flex-col gap-1 px-1 pb-1">
            <button onClick={() => toggleShift(dateStr, 'day')} className={`w-full text-[10px] h-7 rounded-md transition-colors ${shiftData?.day ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-500'}`}>昼:{shiftData?.day ? '◯' : '×'}</button>
            <button onClick={() => toggleShift(dateStr, 'night')} className={`w-full text-[10px] h-7 rounded-md transition-colors ${shiftData?.night ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-500'}`}>夜:{shiftData?.night ? '◯' : '×'}</button>
            <button onClick={() => openTimeModal(dateStr)} disabled={!isDayOrNightSelected} className={`w-full text-[9px] h-7 rounded-md transition-all ${!isDayOrNightSelected ? 'bg-gray-50 text-gray-300 cursor-not-allowed opacity-50' : (hasTime ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-500')}`}>
  ...
  return (
    <div className="w-full max-w-md mx-auto p-4 font-sans text-gray-800 pb-20 select-none">
      ...
      <select value={staffName} onChange={(e) => {setStaffName(e.target.value); localStorage.setItem("shiftApp_staffName", e.target.value);}} className="w-full border-2 p-3 rounded-lg mb-6 bg-white text-[16px]"><option value="">名前を選択</option>{staffList.map((s, i) => <option key={i} value={s.name}>{s.name}</option>)}</select>
      ...
      {/* ★修正: プレースホルダーのテキストを分かりやすく変更 */}
      <textarea 
        value={monthlyRemark} 
        onChange={(e) => setMonthlyRemark(e.target.value)} 
        className="w-full border-2 p-3 rounded-lg mt-6 text-[16px]" 
        placeholder="管理者にのみ表示されます（入力した時のみ上書きされます）" 
        rows={3} 
      />
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-6 rounded-2xl w-full max-w-sm">
            ...
            <div className="flex gap-2 mb-6 justify-center text-lg">
              <select value={tempStartTime} onChange={e => setTempStartTime(e.target.value)} className="border-2 border-gray-300 p-2 rounded-lg bg-white text-[16px]"><option value="">指定なし</option>{activeTimeOptions.map(t => <option key={t} value={t}>{t}</option>)}</select><span className="font-bold text-gray-500 mt-2">〜</span>
              <select value={tempEndTime} onChange={e => setTempEndTime(e.target.value)} className="border-2 border-gray-300 p-2 rounded-lg bg-white text-[16px]"><option value="">指定なし</option>{activeTimeOptions.map(t => <option key={t} value={t}>{t}</option>)}</select>
            </div>
